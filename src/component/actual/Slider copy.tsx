...
export function Slider({ cars }: { cars: AllCarDto }) {
    ...
    // let touchEndX: number | undefined;
    ...
    const [carsSlides, setCarsSlides] = useState<AllCarDto>(
        Array.isArray(cars) && cars.length
            ? Array(16)
                .fill(0)
                .map((el) => cars[Math.floor(Math.random() * cars.length)])
            : []
    );

    const slides = carsSlides.filter((_, idx) => {
    ...
    const handleNextClick = () => {
        setCurrentSlide((prev) =>
            Math.min(prev + 1, Math.floor(carsSlides.length / itemsPerPage) - 1)
        );
    };
            ...
            flex:0 0 calc(100% / 4)
                ...
                @media(max-width: 720px) {
                   ...
                   .cardImg{
                    ...
                    .slides-slides::-webkit-scrollbar {
                        display:none;
                    }
                    .card {
                        flex:0 0 auto;
